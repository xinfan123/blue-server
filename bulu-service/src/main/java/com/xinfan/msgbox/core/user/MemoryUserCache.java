...
public class MemoryUserCache implements UserCache {
	...
	public MemoryUserCache(MessageContext context) {
		...
		for (User user : users) {
			...
			if (sets.size() == 1) {
				...
				// List<CachedMessage> caches = new
				// ArrayList<CachedMessage>(interests.size());
				...
				for (Message msg : interests) {
					...
					cm.setDeadTime(null != msg.getValidTime() ? msg.getValidTime() : TimeHelper.getDefaultDeadTime());
	...
	public boolean updateUserPosition(long userId, Position position) {
		...
		// List<CachedMessage> userMessages =
		// context.getMessagePool().getUserMessage(userId);
	...
	public boolean updateUserInterestsMsg(long userId, CachedMessage message) {
		context.getInterestsCache().updateMessage(context.getInterestsCache().getMessageById(message.getMessageId()), message);
		return true;
	}
	...
	public boolean updateMessage(CachedMessage message) {
		context.getMessagePool().updateMessage(context.getMessagePool().getMessageById(message.getMessageId()), message);
		return true;
	}
